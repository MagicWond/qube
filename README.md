...
- Qube ps still needs to be better developped to show real VM states, for now its not working properly, it will show up but it just shows stopped or running once the PID is saved, which will only occus when starting out Qubes in -d/--detach mode.
